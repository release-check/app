...
Run the JSON matching bridge used by the app repo:

```bash
cargo run -p rc-worker -- match-json < input.json
```
...
crates/rc-worker    JSON matching bridge and future ingestion/indexing worker
```
